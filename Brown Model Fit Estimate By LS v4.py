...
class RadarAltimeterRetracker:
    ...
    def weighted_least_squares_retracking(self, observed_waveform, initial_params,
        ...
        # 只估计SWH
        SWH0 = initial_params[2]

        current_SWH = SWH0
        history = {'SWH': [current_SWH], 'cost': [], 'iterations': 0}
        learning_rate = learning_rate_init

        for iteration in range(max_iterations):
            # 使用固定参数和当前SWH生成模型波形
            model_waveform = self.forward_model.generate_waveform(tau_fixed, P_u_fixed, current_SWH, xi_fixed)
            residuals = observed_waveform - model_waveform

            signal_power = np.maximum(0.01 * P_u_fixed, np.abs(model_waveform))
            ...
            # 计算对SWH的偏导数（雅可比矩阵）
            jacobian_SWH = np.zeros(self.Nt)
            base = model_waveform

            delta_val = 1e-6
            base_SWH = self.forward_model.generate_waveform(tau_fixed, P_u_fixed, current_SWH + delta_val, xi_fixed)
            jacobian_SWH[:] = (base_SWH - base) / delta_val

            # 构建标量更新
            B = jacobian_SWH.reshape(-1, 1)
            P = P_matrix
            V = residuals
            ...
            try:
                ...
                reg = 1e-8
                delta_SWH = np.linalg.solve(left_matrix + reg, right_vector).item()
                if np.abs(delta_SWH) > 5.0:
                    delta_SWH = np.sign(delta_SWH) * 5.0
            except np.linalg.LinAlgError:
                gradient = -2 * B.T @ P @ V
                delta_SWH = -learning_rate * gradient.item() / (np.linalg.norm(gradient) + 1e-12)

            if use_line_search:
                alpha = 1.0
                best_alpha = alpha
                best_cost = current_cost
                best_delta = delta_SWH

                for _ in range(10):
                    test_SWH = current_SWH + alpha * delta_SWH
                    test_waveform = self.forward_model.generate_waveform(tau_fixed, P_u_fixed, test_SWH, xi_fixed)
                    ...
                    if test_cost < best_cost:
                        best_cost = test_cost
                        best_alpha = alpha
                        best_delta = alpha * delta_SWH
                        break
                    alpha *= 0.5
                delta_SWH = best_delta
            else:
                if iteration > 0:
                ...
                if np.abs(delta_SWH) > 1e-12:
                    delta_SWH = learning_rate * np.sign(delta_SWH)

            current_SWH += delta_SWH
            current_SWH = np.clip(current_SWH, 0.01, 20.0)

            history['SWH'].append(current_SWH)
            history['iterations'] = iteration + 1

            if iteration % 20 == 0:
                print(f"迭代 {iteration}: cost={current_cost:.3e}, "
                      f"SWH={current_SWH:.2f}m, "
                      f"lr={learning_rate:.2e}")

        return current_SWH, history
...
print(f"固定参数使用真值: tau={tau_true * 1e9:.2f}ns, P_u={P_u_true:.3f}, xi={np.rad2deg(xi_true):.2f}°")

# 运行改进的加权最小二乘（只估计SWH）
print("\n开始加权最小二乘反演（只估计SWH）...")
start = time.time()
SWH_est, history = retracker.weighted_least_squares_retracking(
    observed_waveform,
    initial_params,
    max_iterations=300,
    tolerance=1e-10,
...
print(f"估计参数: SWH={SWH_est:.2f}m (其他参数使用真值)")
...
plt.xlabel('时间 [ns] (星下点回波 = 0)')
plt.ylabel('归一化功率')
plt.title('最终拟合波形与真实波形对比（只估计SWH）')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
